...
An object-oriented Python implementation for an OOP course project.
It demonstrates abstraction, inheritance, encapsulation, polymorphism,
composition, and simple rule-based automation.
"""
...
class SensorReading:
    """Immutable value object representing one temperature reading."""
...
class SmartDevice(ABC):
    """Abstract base class for every controllable smart device."""
...
class Light(SmartDevice):
    """Smart light that supports ON/OFF control."""
...
class Fan(SmartDevice):
    """Smart fan that supports ON/OFF control."""
...
class AirConditioner(SmartDevice):
    """Air conditioner with ON/OFF control and target temperature."""
...
class TemperatureSensor:
    """Sensor object that stores the latest indoor temperature."""
...
class AutomationRule(ABC):
    """Abstract strategy for sensor-based automation."""
...
class ComfortTemperatureRule(AutomationRule):
    """
    Automatically controls cooling devices based on indoor temperature.

    - Hot: AC and fan ON, AC target set to comfort target.
    - Comfortable: fan ON only for air circulation.
    - Cool: AC and fan OFF.
    """
...
class SmartHomeController:
    """Central controller that manages devices, sensors, and automation rules."""
...
def build_demo_home() -> SmartHomeController:
    """Factory function for a demonstration home."""
...
def run_demo() -> None:
    """Run a deterministic demonstration for screenshots/report output."""
...
def baseline_status_report_without_polymorphism(devices: List[SmartDevice]) -> str:
    """
    Non-polymorphic baseline used for the report's comparative experiment.

    This version must check the concrete class of each device. It works for the
    current three device types, but it becomes harder to maintain when new
    device classes are added.
    """
...
def run_polymorphism_experiment() -> None:
    """
    Compare status reporting with and without polymorphism.
    """
